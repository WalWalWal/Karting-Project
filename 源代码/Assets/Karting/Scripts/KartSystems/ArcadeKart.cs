...
namespace KartGame.KartSystems
{
    public class ArcadeKart : MonoBehaviour
    {
        ...
        public Material rightLight;
        public Material leftLight;

        public Material fireTrial;
        public Material laserTrial;

        private float AccelTime;
        private float boost;
        private float fixSpeed;
        private float fixAccel;
        ...
        void Awake()
        {
            ...
            flame = flame.GetComponent<ParticleSystem>();
            if (flame) flame.Stop();

            AccelTime = 0;
            boost = 0;

            fixSpeed = baseStats.TopSpeed;
            fixAccel = baseStats.Acceleration;
        }
        ...
        void MoveVehicle(float accelInput, float turnInput)
        {
            ...
            if (turnInput != 0 ) currentSpeed *= (fixSpeed - (Mathf.Abs(turningPower) / 2)) / fixSpeed;
            if (currentSpeed > 9) currentSpeed -= Random.value * 15 * Time.deltaTime;
            ...
            if (boostBar)
            {
                if (UnityEngine.Input.GetKey("space"))
                {
                    if (boost > 1)
                    {
                        boost -= Time.deltaTime * 40;
                        boostBar.GetComponent<Image>().fillAmount = boost / 100 * 0.2f;
                        baseStats.TopSpeed = fixSpeed * 1.5f;
                        baseStats.Acceleration = fixAccel * 2;
                        if (flame) flame.Play();
                    }
                    else 
                    { 
                        if (flame) flame.Stop();
                        baseStats.TopSpeed = fixSpeed;
                        baseStats.Acceleration = fixAccel;
                    }
                }
                else if (boost < 100 && currentSpeed > 0)    
                {
            ...
            if (currentSpeed > baseStats.TopSpeed) currentSpeed -= Time.deltaTime; 

            if (UnityEngine.Input.GetKey("s") && !UnityEngine.Input.GetKey("w"))
            {
                backLight.SetColor("_EmissionColor", Color.red);
            } else
            {
                backLight.SetColor("_EmissionColor", Color.black);
            }

            if (turnInput > 0)
            {
                rightLight.SetColor("_EmissionColor", Color.yellow);
            }
            else
            {
                rightLight.SetColor("_EmissionColor", Color.black);
            }

            if (turnInput < 0)
            {
                leftLight.SetColor("_EmissionColor", Color.yellow);
            }
            else
            {
                leftLight.SetColor("_EmissionColor", Color.black);
            }

            if (progressCircle)
            {
                progressCircle.GetComponent<Image>().fillAmount = currentSpeed / (fixSpeed * 2f);
            }

            AccelTime += Time.deltaTime;
            if (AccelTime >= 0.05)
            {
                if (Speed) Speed.text = ((int)(currentSpeed * 5)).ToString();
                AccelTime = 0;
            }
            
            TrailRenderer[] lasers = GetComponentsInChildren<TrailRenderer>();
            if (Mathf.Abs(turningPower) > 4f)
            {
                for (int i = 0; i < lasers.Length; ++i)
                {
                    lasers[i].material = laserTrial;
                }
            }
            else 
            {
